...
namespace Cheetah.Networking
{
    public class AutoSet<T>
    {
        ...
        protected bool NeedsSync => MyAPIGateway.Multiplayer.MultiplayerActive;
        ...
        {
            if (!Networker.Inited) Networker.Init(Radars.SessionCore.ModID);
            this.DataID = DataID;
        ...
        public void Set(T New)
        {
            if (IsValid(New))
            {
                ...
                {
                    Underlying = New;
                    if (NeedsSync) Networker.SendToAll(Header, "Set", Serialize(Underlying), EntityID);
                }
                else
                {
                    if (NeedsSync) Networker.SendToServer(Header, "SetRequest", Serialize(New), EntityID);
                }
            }
        }
        ...
        public void Ask()
        {
            if (NeedsSync && !Networker.IsServer) Networker.SendToServer(Header, "Get", null, EntityID);
        }
    ...
    public static class Networker
    {
        ...
        static void SecondaryHandler(DataMessage Message)
        {
            ...
            if (Message.ObjectID == 0)
            {
                ...
                {
                    Radars.SessionCore.DebugWrite("Networker.SessionHandlers", $"A message with tag '{Message.SenderName}' was received and no handler is subscribed to it.");
                }
            }
            else
            {
                if (EntityHandlersCollection.ContainsKey(Message.ObjectID))
                {
                    ...
                    {
                        Radars.SessionCore.DebugWrite("Networker.EntityHandlers", $"A message with tag '{Message.SenderName}' was received and no handler is subscribed to it.");
                    }
                }
                else
                {
                    Radars.SessionCore.DebugWrite("Networker", $"A message with entityid '{Message.ObjectID}' was received and no handler is subscribed to this entity.");
                }
            }
        }
    ...
    public static class Helper
    {
        ...
        public static List<ulong> GetClientIDs(this VRage.Game.ModAPI.IMyPlayerCollection Collection)
        {
