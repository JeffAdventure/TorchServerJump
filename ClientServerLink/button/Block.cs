...
namespace Cheetah.Radars
{
    ...
    public class BlockLogic : MyGameLogicComponent
    {
        ...
        List<IMyInventory> OnboardInventories = new List<IMyInventory>();
        IMyCubeGrid Grid;
        ...
        private void Tool_AppendingCustomInfo(IMyTerminalBlock trash, StringBuilder Info)
        {
            ...
            {
                Info.AppendLine($"Missing Components:\n");
        ...
        void BuildInventoryCache(bool Force = false)
        {
            ...
            OnboardInventories.Clear();
            OnboardInventories.Add(ToolCargo);
            foreach (var InventoryOwner in InventoryOwners)
            {
                ...
                foreach (var Inventory in InventoryOwner.GetInventories())
                {
                    ...
                    if (Inventory.IsConnectedTo(ToolCargo)) OnboardInventories.Add(Inventory);
                }
            }
        }
        ...
        void Weld(List<IMySlimBlock> Blocks, int ticks = 1)
        {
            ...
            var Onboards = OnboardInventories.Select(x => x.Owner as IMyCubeBlock).ToHashSet();
            foreach (IMySlimBlock Block in Blocks)
            {
                if (Block.CubeGrid.Physics?.Enabled == true)
                {
                    if (!Block.CanContinueBuild(ToolCargo))
                        if (Tool.UseConveyorSystem) ToolCargo.PullAny(Onboards, Block.GetMissingComponents()); else continue;
                ...
                else
                {
                    try
                    {
                        ...
                        if (ToolCargo.PullAny(Onboards, FirstItem.SubtypeName, 1))
                        {
                            try
                            {
